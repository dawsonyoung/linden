package api

import (
	"bytes"
	"net/http"
	"net/http/httptest"
	"testing"
)

func Test_OpenAIChat_InvalidJSON_Returns400(t *testing.T) {
	srv := NewServer("localhost:0", discardLogger(), BuildInfo{}, nil)

	req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions", bytes.NewReader([]byte("{invalid-json")))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()

	srv.Handler.ServeHTTP(w, req)

	if w.Result().StatusCode != http.StatusBadRequest {
		t.Errorf("status = %d, want %d", w.Result().StatusCode, http.StatusBadRequest)
	}
}

func Test_OpenAIChat_MissingRequiredFields_Returns400(t *testing.T) {
	srv := NewServer("localhost:0", discardLogger(), BuildInfo{}, nil)

	// Missing model and messages
	req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions", bytes.NewReader([]byte(`{"stream":true}`)))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()

	srv.Handler.ServeHTTP(w, req)

	if w.Result().StatusCode != http.StatusBadRequest {
		t.Errorf("status = %d, want %d", w.Result().StatusCode, http.StatusBadRequest)
	}
}

func Test_OpenAIChat_NonStreaming_Returns400(t *testing.T) {
	srv := NewServer("localhost:0", discardLogger(), BuildInfo{}, nil)

	// Stream is false
	req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions", bytes.NewReader([]byte(`{"model":"tinyllama","messages":[{"role":"user","content":"hello"}],"stream":false}`)))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()

	srv.Handler.ServeHTTP(w, req)

	if w.Result().StatusCode != http.StatusBadRequest {
		t.Errorf("status = %d, want %d", w.Result().StatusCode, http.StatusBadRequest)
	}
}
